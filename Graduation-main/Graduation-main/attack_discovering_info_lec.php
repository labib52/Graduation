...
- which is the graphical user interface of Nmap. So to run it in terminal, you just have to type Zenmap or you can find it under your applications menu.
 ...
 you can see the command. This is actually the Nmap command that will be executed when I hit this Command button. So like I said, Zenmap, what we're using right now is just the graphical interface that will run this Nmap command in the background and show me the results.</p>
<p>Alternatively, if you don't really know much about Nmap and its commands, you can use one of their ID profiles in here. So in this lecture, we're actually gonna be using a number of these profiles and we'll see the difference between them in terms of speed and the information gathered.
...
So again, it was a very quick scan, but as you can see, it still gave us much more information than what we got from Netdiscover.</p>
<p>-The next scan that I wanna show you is the Quick scan.
Now this is gonna be slightly slower than the Ping scan but it's gonna show us more information. So right now you can see that the scan is showing us the same information that we've seen *before with the ping scan, but it's also showing us the open ports on each one of the discover devices.</p>
<p>the next scan is the quick scan plus.
...
command: ssh root@192.168.1.12</p>
